...
def compute_distances(X1, X2):
    ...
    X1_X2 = np.dot(X1, np.transpose(X2))
    
    dists = np.sqrt(X1_square.reshape(-1,1) - 2 * X1_X2 + X2_square)
    # END YOUR CODE
...
def predict_labels(dists, y_train, k=1):
    ...
    for i in range(num_test):
        ...
        closest_y = y_train[k_nearest_index]
...
def split_folds(X_train, y_train, num_folds):
    ...
    for i in range(num_folds):    
        ...
        y_vals[i,:] = y_lst[i]
        y_trains[i,:] = np.vstack(y_lst[:i] + y_lst[i+1:]).reshape(1,-1)
    # END YOUR CODE
