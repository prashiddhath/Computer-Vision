...
def load_dataset(dir):
    ...
    # Load all the images under 'dir/imgs' and corresponding
    # segmentation masks under 'dir/gt'.
    for fname in sorted(os.listdir(os.path.join(dir, 'imgs'))):
        if fname.endswith('.jpg'):
            # Load image
            img = io.imread(os.path.join(dir, 'imgs', fname))
            imgs.append(img)

            # Load corresponding gt segmentation mask
            mask_fname = fname[:-4] + '.png'
            gt_mask = io.imread(os.path.join(dir, 'gt', mask_fname))
